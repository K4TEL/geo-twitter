...
print(df.user.mode())
n = 100
users = df['user'].value_counts()[:n].index.tolist()

for u in users:
    print((df.user == u).sum(), u)

user_df = df[df['user'].isin(users)]
with open("datasets/worldwide-twitter-day-users.jsonl", "w") as f:
